...
afterEach(() => { vi.doUnmock("keytar"); vi.resetModules(); });
...
describe("lazy credential store", () => {
  it("loads the native module only when a store operation needs it", async () => {
    ...
    const factory = vi.fn(() => ({ default: { getPassword, setPassword, deletePassword } }));
    vi.doMock("keytar", factory);
    const { createKeytarStore, isKeytarAvailable } = await import("./keytar-store.js");
    const store = createKeytarStore();
    expect(factory).not.toHaveBeenCalled();
    expect(await isKeytarAvailable()).toBe(true);
  ...
  it("keeps help/version usable and reports actionable auth errors without native libraries", async () => {
    vi.doMock("keytar", () => { throw new Error("missing native library"); });
    ...
    const { createKeytarStore, isKeytarAvailable } = await import("./keytar-store.js");
    expect(await isKeytarAvailable()).toBe(false);
    await expect(createKeytarStore().load()).rejects.toThrow(/libsecret-1-0/);
